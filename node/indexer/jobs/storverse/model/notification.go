...
func UpdateNotificationReadStatus(ctx context.Context, db *sql.DB) (int64, error) {
	updateQuery := `UPDATE NOTIFICATION 
		...
		AND EXISTS (
			...
			AND (
				(READ_NOTIFICATIONS.MESSAGETYPE = 2 AND NOTIFICATION.MESSAGETYPE IN (2, 3))
				OR (READ_NOTIFICATIONS.MESSAGETYPE = 4 AND NOTIFICATION.MESSAGETYPE IN (4, 7))
				OR (READ_NOTIFICATIONS.MESSAGETYPE = 5 AND NOTIFICATION.MESSAGETYPE IN (5, 6))
				OR (READ_NOTIFICATIONS.MESSAGETYPE NOT IN (2, 4, 5) AND NOTIFICATION.MESSAGETYPE = READ_NOTIFICATIONS.MESSAGETYPE)
			)
